...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.querySelectorAll('a, button, .grid-item, #nav-trigger, input, audio, .work-image-container').forEach(el => {
        el.addEventListener('mouseover', () => {
            ...
            cursor.style.boxShadow = '0 0 20px rgba(228, 161, 193, 0.5), inset 0 0 12px rgba(255, 255, 255, 0.4)';
        });
        el.addEventListener('mouseout', () => {
            if (!pointerFine || !cursor) return;
            cursor.style.width = '36px';
            cursor.style.height = '36px';
            cursor.style.boxShadow = '0 0 12px rgba(228, 161, 193, 0.3), inset 0 0 8px rgba(255, 255, 255, 0.3)';
        });
    });
